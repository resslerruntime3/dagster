...
class ToposortedPriorityQueue:
    ...
    class QueueItem(NamedTuple):
        level: int
        partition_ordinal: Optional[float]
        asset_partition: AssetKeyPartitionKey

        def __eq__(self, other):
            return self.level == other.level and self.partition_ordinal == other.partition_ordinal

        def __lt__(self, other):
            return self.level < other.level or (
                self.level == other.level
                and self.partition_ordinal is not None
                and self.partition_ordinal < other.partition_ordinal
            )
    ...
    def _queue_item(self, asset_partition: AssetKeyPartitionKey):
        ...
        return ToposortedPriorityQueue.QueueItem(level, partition_ordinal, asset_partition)
